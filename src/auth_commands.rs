    ...
    let cmd = Command::new("gws auth")
        ...
        .arg(
            Arg::new("account")
                .long("account")
                .short('A')
                .help("Account alias to use (e.g. 'work', 'personal')")
                .global(true),
        )
        .subcommand(
            Command::new("login")
    ...
    match matches.subcommand() {
        Some(("login", sub)) => {
            let mut opts = LoginOptions {
                readonly: sub.get_flag("readonly"),
                full: sub.get_flag("full"),
                ..Default::default()
            };
            if let Some(s) = sub.get_one::<String>("scopes") {
                opts.custom_scopes = Some(s.split(',').map(|s| s.trim().to_string()).collect());
            }
            if let Some(s) = sub.get_one::<String>("services") {
                opts.services_filter = Some(
                    s.split(',')
            ...
            run_login(opts, account).await
        }
        Some(("setup", sub)) => {
            let opts = crate::setup::SetupOptions {
                project: sub.get_one::<String>("project").cloned(),
                dry_run: sub.get_flag("dry-run"),
                login: sub.get_flag("login"),
        ...
        Some(("export", sub)) => {
            let unmasked = sub.get_flag("unmasked");
            handle_export(unmasked, account).await
        }
        Some(("logout", _)) => handle_logout(account),
...
pub struct LoginOptions {
    ...
    pub services_filter: Option<HashSet<String>>,
}

/// Run the `auth login` flow.
///
/// Exposed for internal orchestration (e.g. `auth setup --login`).
pub async fn run_login(opts: LoginOptions, account: Option<&str>) -> Result<(), GwsError> {
    handle_login(opts, account).await
}
...
async fn handle_login(opts: LoginOptions, account: Option<&str>) -> Result<(), GwsError> {
    ...
    if token.token().is_some() {
        ...
        let enc_path = credential_store::save_encrypted(&creds_str, account)
...
async fn handle_export(unmasked: bool, account: Option<&str>) -> Result<(), GwsError> {
    ...
    match credential_store::load_encrypted(account) {
        Ok(contents) => {
            if unmasked {
                println!("{contents}");
...
async fn resolve_scopes(
    ...
    if let Some(ref scopes) = opts.custom_scopes {
        return scopes.clone();
    }

    if opts.readonly {
        let scopes: Vec<String> = READONLY_SCOPES.iter().map(|s| s.to_string()).collect();
        let mut result = filter_scopes_by_services(scopes, opts.services_filter.as_ref());
        augment_with_dynamic_scopes(&mut result, opts.services_filter.as_ref(), true).await;
        return result;
    }
    if opts.full {
        let scopes: Vec<String> = FULL_SCOPES.iter().map(|s| s.to_string()).collect();
        let mut result = filter_scopes_by_services(scopes, opts.services_filter.as_ref());
        augment_with_dynamic_scopes(&mut result, opts.services_filter.as_ref(), false).await;
        return result;
    }
    ...
    if !cfg!(test) && std::io::IsTerminal::is_terminal(&std::io::stdin()) {
            ...
            if !enabled_apis.is_empty() {
                ...
                if !scopes.is_empty() {
                    if let Some(selected) =
                        run_discovery_scope_picker(&scopes, opts.services_filter.as_ref())
                    {
                        return selected;
                    }
                }
        ...
        if let Some(selected) = run_simple_scope_picker(opts.services_filter.as_ref()) {
            return selected;
        }
    }

    let defaults: Vec<String> = DEFAULT_SCOPES.iter().map(|s| s.to_string()).collect();
    let mut result = filter_scopes_by_services(defaults, opts.services_filter.as_ref());
    augment_with_dynamic_scopes(&mut result, opts.services_filter.as_ref(), false).await;
    result
}
...
async fn handle_status(account: Option<&str>) -> Result<(), GwsError> {
    let plain_path = plain_credentials_path(account);
    let enc_path = credential_store::encrypted_credentials_path(account);
...
fn handle_logout(account: Option<&str>) -> Result<(), GwsError> {
    let plain_path = plain_credentials_path(account);
    let enc_path = credential_store::encrypted_credentials_path(account);
    let token_cache = token_cache_path(account);
    ...
    crate::timezone::invalidate_cache(account);
...
mod tests {
    ...
    fn run_resolve_scopes(opts: LoginOptions, project_id: Option<&str>) -> Vec<String> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(resolve_scopes(&opts, project_id))
    }
    ...
    fn run_resolve_scopes_with_services(
        ...
        opts.services_filter = Some(filter);
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(resolve_scopes(&opts, project_id))
    }
    ...
    fn resolve_scopes_returns_defaults_when_no_flag() {
        let scopes = run_resolve_scopes(LoginOptions::default(), None);
    ...
    fn resolve_scopes_returns_custom_scopes() {
        let opts = LoginOptions {
            custom_scopes: Some(vec!["https://www.googleapis.com/auth/drive.readonly".to_string()]),
            ..LoginOptions::default()
        };
        let scopes = run_resolve_scopes(opts, None);
    ...
    fn resolve_scopes_ignores_trailing_flag() {
        // In the refactored version, LoginOptions is pre-populated.
        // If custom_scopes is None, it should use defaults.
        let scopes = run_resolve_scopes(LoginOptions::default(), None);
        assert_eq!(scopes.len(), DEFAULT_SCOPES.len());
    }
    ...
    fn resolve_scopes_custom_overrides_readonly() {
        ...
        let opts = LoginOptions {
            custom_scopes: Some(vec!["https://www.googleapis.com/auth/drive".to_string()]),
            readonly: true,
            ..LoginOptions::default()
        };
        let scopes = run_resolve_scopes(opts, None);
    ...
    fn plain_credentials_path_defaults_to_config_dir() {
        ...
        let path = plain_credentials_path(None);
    ...
    fn token_cache_path_is_in_config_dir() {
        let path = token_cache_path(None);
    ...
    async fn handle_auth_command_empty_args_prints_usage() {
        ...
        // clap returns an error of kind DisplayHelp for required subcommands
        assert!(result.is_err());
        if let GwsError::Validation(e) = result.unwrap_err() {
            assert!(e.contains("Usage") || e.contains("gws auth"));
        } else {
    ...
    async fn handle_auth_command_help_flag_returns_ok() {
        ...
        let result = handle_auth_command(&args).await;
        assert!(result.is_err()); // clap returns ErrorKind::DisplayHelp
    }
    ...
    fn resolve_scopes_with_services_filter() {
        let scopes = run_resolve_scopes_with_services(LoginOptions::default(), None, &["drive", "gmail"]);
    ...
    fn resolve_scopes_services_filter_unknown_service_ignored() {
        let scopes = run_resolve_scopes_with_services(LoginOptions::default(), None, &["drive", "nonexistent"]);
        assert!(!scopes.is_empty());
    ...
    fn resolve_scopes_services_takes_priority_with_readonly() {
        ...
        let scopes = run_resolve_scopes_with_services(opts, None, &["drive"]);
        assert!(!scopes.is_empty());
        for scope in &scopes {
            let short = scope
                .strip_prefix("https://www.googleapis.com/auth/")
    ...
    fn resolve_scopes_services_takes_priority_with_full() {
        ...
        let scopes = run_resolve_scopes_with_services(opts, None, &["gmail"]);
        assert!(!scopes.is_empty());
        for scope in &scopes {
            let short = scope
                .strip_prefix("https://www.googleapis.com/auth/")
    ...
    fn resolve_scopes_explicit_scopes_bypass_services_filter() {
        ...
        let opts = LoginOptions {
            custom_scopes: Some(vec!["https://www.googleapis.com/auth/calendar".to_string()]),
            ..LoginOptions::default()
        };
        let scopes = run_resolve_scopes_with_services(opts, None, &["drive"]);
        assert_eq!(scopes.len(), 1);
